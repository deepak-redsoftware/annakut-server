...
class UserService extends CrudRepository {
        ...
        try {
            ...
            await session.withTransaction(async () => {
                if (await Counter.findOne({ id: 'userID' }) !== null) {
                    counter = await Counter.findOneAndUpdate({ id: 'userID' }, {"$inc": {"seq": 1}}, {new: true}).session(session).exec();
                } else {
                    counter = await Counter.create([{ id: 'userID', seq: USER_ID_START}], { session: session });
    ...
    signIn(user) {
        try {
            const token = jwt.sign({ userID: user.userID, id: user._id, role: user.role }, JWT_SECRET, {
                expiresIn: '5h'
            });
            return token;
        } catch (error) {
    ...
    async getLoggedInUser(user) {
        try {
            const foundUser = await User.findById(user.id).select('userID -_id role').exec();
            return foundUser;
        } catch (error) {
            console.error(`Error at user service layer: ${error}`);
            throw error;
